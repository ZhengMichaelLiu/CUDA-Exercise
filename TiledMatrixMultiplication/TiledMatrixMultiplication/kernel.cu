...
cudaError_t tileMatrixMultiplicationWithCUDA(float* matrixA, float* matrixB, float* matrixC,
...
__global__ void tileMatrixMultiplicationKernel(float* matrixA, float* matrixB, float* matrixC,
											   int matrixARows, int matrixACols,
											   int matrixBRows, int matrixBCols,
											   int matrixCRows, int matrixCCols) {
	...
	for (int phase = 0; phase < ceil(1.0 * matrixACols / BLOCK_SIZE); phase++) {
		if (row < matrixARows && phase * BLOCK_SIZE + threadX < matrixACols) {
			subTileA[threadY][threadX] = matrixA[row * matrixACols + phase * BLOCK_SIZE + threadX];
		} else {
			subTileA[threadY][threadX] = 0.0f;
		}

		if (col < matrixBCols && phase * BLOCK_SIZE + threadY < matrixBRows) {
			subTileB[threadY][threadX] = matrixB[(phase * BLOCK_SIZE + threadY) * matrixBCols + col];
		} else {
			subTileB[threadY][threadX] = 0.0f;
		}
		__syncthreads();

		for (int k = 0; k < BLOCK_SIZE; k++) {
			Cval += subTileA[threadY][k] * subTileB[k][threadX];
		}
		__syncthreads();
	}

	if (row < matrixCRows && col < matrixCCols) {
		matrixC[row * matrixCCols + col] = Cval;
	}
}
...
int main() {
	...
	cudaError_t cudaStatus = tileMatrixMultiplicationWithCUDA(matrixA, matrixB, matrixC,
															  matrixARows, matrixACols,
															  matrixBRows, matrixBCols,
															  matrixCRows, matrixCCols);
...
cudaError_t tileMatrixMultiplicationWithCUDA(float* matrixA, float* matrixB, float* matrixC,
	...
	tileMatrixMultiplicationKernel << <dimGrid, dimBlock >> > (dev_a, dev_b, dev_c,
															   matrixARows, matrixACols,
															   matrixBRows, matrixBCols,
															   matrixCRows, matrixCCols);
